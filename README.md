...
Server 菜单 **55 Extra inbounds**（主 VMess 不变）：VLESS+REALITY、VLESS+WS+TLS（可同域名、端口必须不同，nginx 单独写 `vless-ws.conf`，UUID/path 按主 VMess 生成）、SOCKS5、dokodemo 多规则。操作流程：[docs/server-menu-55.md](docs/server-menu-55.md)。进入 `bash /opt/de_GWD/server` → 55，`[5]` 查看已添加项，`[0]` 返回。
